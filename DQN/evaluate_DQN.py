...
os.environ['CUDA_VISIBLE_DEVICES'] = '-1'  # Disabling GPU for evaluation
...
if __name__ == "__main__":
    ...
    model_id = 0
    max_rwd = 0
    print(f"File: {args.d[0]}")
    with open(args.d[0]) as fp:
        for line in fp:
            arrayLine = line.split(":")
            if arrayLine[0]=='MAX REWD':
                rwd = float(arrayLine[1].split(" ")[1])
                if rwd > max_rwd:
                    model_id = int(arrayLine[2].split(",")[0])
                    max_rwd = rwd
                    print(f"Found higher rwd: Model: {model_id} {rwd}")


    print(f"Best performing model: {model_id}")
